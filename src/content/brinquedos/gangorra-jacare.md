---
title: "Gangorra Jacaré"
description: "Gangorra inflável em formato de jacaré para diversão em dupla das crianças no Rio de Janeiro."
icon: "🐊"
popular: false
minAge: "3 anos"
tags: ["inflavel", "gangorra", "jacare"]
---

# Gangorra Jacaré

A Gangorra Jacaré é a diversão em dupla que as crianças adoram. Em formato de jacaré colorido, dois participantes se balançam juntos de forma segura e divertida. Ideal para festas infantis e eventos com crianças pequenas.

## Características

- Gangorra inflável em formato de jacaré
- Assentos duplos com encosto
- Base estável e antiderrapante
- Cores vibrantes e chamativas
- Resistente e durável

## Ideal para

- Festas de aniversário infantil
- Eventos escolares
- Festas de 3 a 7 anos
- Eventos em condomínios

## Segurança

Base antiderrapante. Movimento controlado. Peso máximo de 60kg total. Uso supervisionado recomendado.
## Especificações Técnicas

- Dimensões e especificações disponíveis sob consulta
- Capacidade de acordo com a faixa etária recomendada
- Material reforçado e certificado para uso em eventos

## Diferenciais

- Higienização completa antes de cada evento
- Montagem e retirada inclusas no valor
- Equipe treinada para instalação segura
- Estrutura revisada e com manutenção preventiva mensal

## Requisitos do Local

- Espaço livre compatível com o tamanho do brinquedo
- Energia: consulte necessidade específica
- Superfície plana e firme
- Acesso adequado para transporte e montagem

## Preço e Orçamento

O Gangorra Jacaré está disponível por **sob consulta**. O valor inclui entrega, montagem, higienização e retirada.

> 💰 **Quer economizar?** Monte um pacote com 3 ou mais brinquedos e ganhe desconto especial de até 20%.
>
> 📱 [Solicite seu orçamento pelo WhatsApp](https://wa.me/5521997751577?text=Olá,+gostaria+de+um+orçamento+para+o+Gangorra%20Jacar%C3%A9) — resposta em até 30 minutos.
