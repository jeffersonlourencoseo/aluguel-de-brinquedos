---
title: "PlayStation 5"
description: "Videogame PlayStation 5 com jogos populares, telas grandes e cadeiras gamer para eventos."
icon: "🎮"
popular: true
minAge: "6 anos"
tags: ["eletronico", "videogame", "games"]
---

# PlayStation 5

Leve a diversão dos games de última geração para seu evento com o PlayStation 5. Com gráficos incríveis, jogos populares e toda a estrutura inclusa, é a atração perfeita para entreter crianças, jovens e adultos.

## Características

- Console PlayStation 5
- Tela de alta definição
- Cadeiras gamer confortáveis
- Jogos populares inclusos
- Controles sem fio DualSense

## Ideal para

- Festas de aniversário
- Eventos corporativos
- Festas de adolescentes
- Campeonatos e torneios

## Segurança

Equipamentos de uso supervisionado. Jogos classificados por faixa etária. Instalação profissional com proteção de cabos.
## Especificações Técnicas

- Dimensões: 0,8m x 1,5m x 1,9m (L x P x A)
- Peso: 120kg
- Tela: 32 polegadas Full HD
- Jogos: 60 jogos clássicos pré-instalados

## Diferenciais

- Mais de 60 jogos clássicos em uma única máquina
- Design retrô autêntico que encanta adultos e crianças
- Não precisa de internet para funcionar
- Controles arcade originais com joystick e 6 botões

## Requisitos do Local

- Espaço: 1,5m x 1,5m
- Energia: 1 tomada 110V/220V
- Pode ser usado em ambientes internos e externos cobertos
- Não expor à chuva

## Preço e Orçamento

O PlayStation 5 está disponível por **a partir de R$ 250/dia**. O valor inclui entrega, montagem, higienização e retirada.

> 💰 **Quer economizar?** Monte um pacote com 3 ou mais brinquedos e ganhe desconto especial de até 20%.
>
> 📱 [Solicite seu orçamento pelo WhatsApp](https://wa.me/5521997751577?text=Olá,+gostaria+de+um+orçamento+para+o+PlayStation%205) — resposta em até 30 minutos.
