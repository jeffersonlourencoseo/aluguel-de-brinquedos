---
title: "Guerra de Cotonetes"
description: "Arena inflável para guerra de cotonetes gigantes, diversão e competição garantidas no Rio de Janeiro."
icon: "🥊"
popular: false
minAge: "6 anos"
tags: ["inflavel", "competicao", "cotonetes"]
---

# Guerra de Cotonetes

A Guerra de Cotonetes é uma arena inflável onde os participantes lutam com cotonetes gigantes acolchoados. É diversão garantida com muita competição e risadas para todas as idades.

## Características

- Arena inflável circular
- Cotonetes gigantes acolchoados
- Capacidade para 2 a 4 competidores
- Plataformas individuais
- Material reforçado e durável

## Ideal para

- Festas de adolescentes
- Eventos corporativos e team building
- Festas de aniversário
- Eventos escolares

## Segurança

Cotonetes totalmente acolchoados. Arena com piso inflável e macio. Laterais altas para proteção. Supervisão obrigatória.
## Especificações Técnicas

- Dimensões e especificações disponíveis sob consulta
- Capacidade de acordo com a faixa etária recomendada
- Material reforçado e certificado para uso em eventos

## Diferenciais

- Higienização completa antes de cada evento
- Montagem e retirada inclusas no valor
- Equipe treinada para instalação segura
- Estrutura revisada e com manutenção preventiva mensal

## Requisitos do Local

- Espaço livre compatível com o tamanho do brinquedo
- Energia: consulte necessidade específica
- Superfície plana e firme
- Acesso adequado para transporte e montagem

## Preço e Orçamento

O Guerra de Cotonetes está disponível por **sob consulta**. O valor inclui entrega, montagem, higienização e retirada.

> 💰 **Quer economizar?** Monte um pacote com 3 ou mais brinquedos e ganhe desconto especial de até 20%.
>
> 📱 [Solicite seu orçamento pelo WhatsApp](https://wa.me/5521997751577?text=Olá,+gostaria+de+um+orçamento+para+o+Guerra%20de%20Cotonetes) — resposta em até 30 minutos.
