---
title: "Giroscópio (Simulador NASA)"
description: "Giroscópio humano que simula a rotação de astronautas, uma experiência única de diversão no Rio de Janeiro."
icon: "🚀"
popular: false
minAge: "8 anos"
tags: ["mecanico", "giroscopio", "aventura"]
---

# Giroscópio (Simulador NASA)

O Giroscópio, também conhecido como Simulador NASA, é uma estrutura metálica onde uma pessoa é presa em um arnês e gira em 360 graus nos três eixos. É uma experiência única que simula a rotação de astronautas no espaço.

## Características

- Estrutura metálica resistente
- Giro em 360 graus em três eixos
- Arnês profissional com mosquetão
- Operador treinado
- Controle manual de velocidade

## Ideal para

- Festas de adolescentes
- Eventos corporativos
- Festas de formatura
- Eventos temáticos de espaço

## Segurança

Arnês profissional com dupla trava. Operador treinado e certificado. Peso máximo de 100kg. Contra-indicado para pessoas com problemas de coluna ou vertigem.
## Especificações Técnicas

- Dimensões e especificações disponíveis sob consulta
- Capacidade de acordo com a faixa etária recomendada
- Material reforçado e certificado para uso em eventos

## Diferenciais

- Higienização completa antes de cada evento
- Montagem e retirada inclusas no valor
- Equipe treinada para instalação segura
- Estrutura revisada e com manutenção preventiva mensal

## Requisitos do Local

- Espaço livre compatível com o tamanho do brinquedo
- Energia: consulte necessidade específica
- Superfície plana e firme
- Acesso adequado para transporte e montagem

## Preço e Orçamento

O Giroscópio (Simulador NASA) está disponível por **sob consulta**. O valor inclui entrega, montagem, higienização e retirada.

> 💰 **Quer economizar?** Monte um pacote com 3 ou mais brinquedos e ganhe desconto especial de até 20%.
>
> 📱 [Solicite seu orçamento pelo WhatsApp](https://wa.me/5521997751577?text=Olá,+gostaria+de+um+orçamento+para+o+Girosc%C3%B3pio%20(Simulador%20NASA)) — resposta em até 30 minutos.
